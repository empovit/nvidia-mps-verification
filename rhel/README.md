...
3. (Optionally) Set `CUDA_MPS_LOG_DIRECTORY` to override the default location at `/var/log/nvidia-mps`.

4. (Optionally) Set other [environment variable](https://docs.nvidia.com/deploy/mps/#environment-variables),
...
6. Start the MPS control daemon. Use `-f` to run it in the foreground to see what's going on.
...
7. Run clients. You can find example shell scripts in this directory.

8. Observe client and server processes in `nvidia-smi`, e.g.

   ```console
   watch -n5 nvidia-smi
   ```

9. Shut down the control daemon.
